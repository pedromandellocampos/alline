...
const has = (ex, subex) => {
  ...
  for (let i = 0; i < ex.length; ++i) if (has(ex[i], subex)) return true;
  return false;
};
const subst = (ex, sub) => {
  if (isAtom(ex)) {
...
const match = (ex, pat, cap) => {
  const matchR = (ex, pat, cap) => {
    if ((isLiteral(pat) && isSymbol(ex)) || (isLiteral(ex) && isSymbol(pat)))
    ...
    for (let i = 1; i < pat.length; ++i) {
      ...
      if (
        (kpi === "BlankNullSequence" || kpi === "BlankSequence") &&
        i != pat.length - 1
      )
...
const flatten = (e) => {
  if (isAtom(e)) return e;
  for (let i = 0; i < e.length; ++i) e[i] = flatten(e[i]);
  const k = kind(e);
  if (k === "compound") return e;
...
const parse = (str) => {
  ...
  Exp = (p) => {
    ...
    while (lexer.isBinary(next()) && lexer.prec(lexer.binary(next())) >= p) {
      const op = lexer.binary(next());
      consume();
      const q = lexer.prec(op) + (lexer.associativity(op) === "Right" ? 0 : 1);
      const t1 = Exp(q);
  ...
  P = () => {
    if (lexer.isUnary(next())) {
      ...
      const t = Exp(q);
      if (op === "UnaryMinus" && kind(t) === "Integer") return Integer(-t[1]);
...
const toLisp = (e) => {
  ...
  let s = "(";
  s += toLisp(e[0]);
  for (let i = 1; i < e.length; ++i) s += " " + toLisp(e[i]);
  s += ")";
  return s;
};
const less = (a, b) => {
...
const Evald = (e) => {
  ...
  if (ke === "compound") {
    const ex = copy(e);
    for (let i = 1; i < ex.length; ++i) ex[i] = Eval(ex[i]);
    let tex;
...
const addRule = (rule, fn, up) => {
  ...
  const rulePush = (r) => {
    if (up) {
      ...
      if (kind(rule) !== "compound") downValuesAdd(s, r);
      else subValuesAdd(s, r);
    }
  };
...
//Functional
addRule($$`Lambda(a_Literal, b_)(c_)`, ({ a, b, c }) => {
  const substList = {};
  substList[a[1]] = c;
  return Eval(subst(b, substList));
});
addRule($$`Lambda(List(a__Literal), b_)(c__)`, ({ a, b, c }) => {
  const substList = {};
...
addRule($$`Def(List(a__Literal), b_)`, ({ a, b }) => {
  const r = Eval(b);
  for (let i = 1; i < a.length; ++i) {
    let s = lookup(a[i][1]);
    if (!s) s = newSymbol(a[i][1]);
    if (i >= b.length) ownValueSet(s, Null);
    else ownValueSet(s, r[i]);
  }
  return r;
});
addRule($$`Def(At(a_Literal(b_)), c_)`, ({ a, b, c }) => {
  let s = lookup(a[1]);
  if (!s) s = newSymbol(a[1]);
  const v = ownValue(s);
  const i = Eval(b);
...
addRule($$`Def(At(a_Literal(b__)), c_)`, ({ a, b, c }) => {
  let s = lookup(a[1]);
  if (!s) s = newSymbol(a[1]);
  let v = ownValue(s);
  let w = v;
...
addRule($$`Def(a_, b_)`, ({ a, b }) => {
  addRule(a, b);
  return Null;
});
addRule($$`Tag(t_Literal, Def(a_, b_))`, ({ t, a, b }) => {
  addRule(a, b, t[1]);
  return Null;
});
addRule($$`Tag(t_Symbol, Def(a_, b_))`, ({ t, a, b }) => {
  addRule(a, b, t[1]);
  return Null;
});
addRule($$`At(a_Literal(b_))`, ({ a, b }) => {
  const s = lookup(a[1]);
  if (!s) {
    console.log(a[1]);
...
addRule($$`Postfix(a_, b_)`, ({ a, b }) => {
  return Cons(b)(a);
});
addRule($$`Map(f_, a_)`, ({ f, a }) => {
  let r = copy(a);
  for (let i = 1; i < a.length; ++i) r[i] = Eval(Cons(f)(a[i]));
  return r;
});
addRule($$`Reduce(f_, a_)`, ({ f, a }) => {
  let r = a[1];
  for (let i = 2; i < a.length; ++i) r = Eval(Cons(f)(r, a[i]));
  return r;
});
addRule($$`Reduce(f_, a_, s_)`, ({ f, a, s }) => {
  let r = s;
  for (let i = 1; i < a.length; ++i) r = Eval(Cons(f)(r, a[i]));
  return r;
});
addRule($$`Apply(f_, a_)`, ({ f, a }) => {
  a[0] = f;
  return a;
});
addRule($$`Append(a_, b_)`, ({ a, b }) => {
  const aa = copy(a);
  aa.push(b);
  return aa;
});
...
addRule($$`Subst(e_, List(s__Tag))`, ({ e, s }) => {
  const substList = {};
  for (let i = 1; i < s.length; ++i) substList[s[i][1][1]] = s[i][2];
  const r = subst(e, substList);
  return r;
});
let blockCounter = 0;
addRule($$`Block(List(vars__), e_)`, ({ vars, e }) => {
  const substList = {};
  ...
  stack.push({});
  for (let i = 1; i < vars.length; ++i) {
    if (kind(vars[i]) === "Literal") newSymbol(vars[i][1]);
    if (kind(vars[i]) === "Def") {
      const name = vars[i][1][1];
...
addRule($$`Table(e_, List(v_Literal, a_, b_))`, ({ e, v, a, b }) => {
  const r = List();
...
addRule($$`ClearAll()`, ({}) => {
  for (let v in gglobal) delete gglobal[v];
  return Null;
});
addRule($$`Clear(v__Literal)`, ({ v }) => {
  for (let i = 1; i < v.length; ++i) delete gglobal[v[i][1]];
  return Null;
});
addRule($$`Print(e__)`, ({ e }) => {
  for (let i = 1; i < e.length; ++i) console.log(toString(e[i]));
  return Null;
});
addRule($$`ToString(e_)`, ({ e }) => {
...
addRule($$`In(a_, b_Set)`, ({ a, b }) => {
  for (let i = 1; i < b.length; ++i) {
    if (equal(a, b[i])) return True;
  }
  return False;
});
addRule($$`Union(a_Set, b_Set)`, ({ a, b }) => {
  const r = a.slice(1).concat(b.slice(1));
  return Set(...r);
});
addRule($$`Intersection(a_Set, b_Set)`, ({ a, b }) => {
  const r = [];
  let i = 1;
  let j = 1;
...
addRule($$`Equal(a_,b_)`, ({ a, b }) => {
  if (equal(a, b)) return True;
  return False;
});
addRule($$`Less(a_,b_)`, ({ a, b }) => {
  if (less(a, b)) return True;
  return False;
});
addRule($$`LessEqual(a_,b_)`, $$`Or(Less(a, b), Equal(a, b))`);
addRule($$`Great(a_,b_)`, $$`Less(b, a)`);
addRule($$`GreatEqual(a_,b_)`, $$`Or(Less(b, a), Equal(a, b))`);
addRule($$`Kind(a_)`, ({ a }) => {
  const ka = kind(a);
...
addRule($$`Plus()`, $$`0`);
addRule($$`Plus(a_)`, $$`a`);
addRule($$`Plus(0, a__)`, $$`Plus(a)`);
addRule($$`Plus(a_Integer, b_Integer, c___)`, ({ a, b, c }) =>
  Plus(a[1] + b[1], c)
);
addRule($$`UnaryMinus(a_Integer)`, ({ a }) => Integer(-a[1]));
addRule($$`UnaryMinus(a_)`, $$`Times(-1, a)`);
addRule($$`Subtract(a_, b_)`, $$`Plus(a, Times(-1, b))`);
addRule($$`Times()`, $$`1`);
addRule($$`Times(a_)`, $$`a`);
addRule($$`Times(1, a__)`, $$`Times(a)`);
addRule($$`Times(0, a__)`, $$`0`);
addRule($$`Times(a_Integer, b_Integer, c___)`, ({ a, b, c }) =>
  Times(a[1] * b[1], c)
);
addRule($$`Power(_, 0)`, $$`1`);
addRule($$`Power(1, _)`, $$`1`);
addRule($$`Power(a_, 1)`, $$`a`);
addRule($$`Power(a_Integer, b_Integer)`, ({ a, b }) => {
  if (b[1] > 0) return Integer(Math.pow(a[1], b[1]));
  return null;
});
addRule($$`Quotient(a_Integer, b_Integer)`, ({ a, b }) => {
  return Integer(Math.floor(a[1] / b[1]));
});
addRule($$`Mod(a_Integer, b_Integer)`, ({ a, b }) => {
