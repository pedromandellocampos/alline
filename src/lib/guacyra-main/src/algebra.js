...
const isNumericEx = (e) => {
	...
	for (let i = 1; i < e.length; ++i) if (!isNumericEx(e[i])) return false;
	return true;
};
addRule($$`Rational(a_Integer, b_Integer)`, ({ a, b }) => {
	let num = a[1];
	let den = b[1];
	const g = gcd(num, den);
...
addRule($$`Power(a_Integer, b_Rational)`, ({ a, b }) => {
	...
	if (b[1][1] > 0) {
		...
		if (u == 1 && b[1][1] == 1) return null;
		else return Times(u, Power(v, Rational(1, b[2][1])));
	} else {
		const b1 = -b[1][1];
...
addRule($$`Numerator(a_Integer)`, $$`a`);
addRule($$`Numerator(Rational(a_Integer, b_Integer))`, $$`a`);
addRule($$`Numerator(a_)`, ({ a }) => {
	const r = Eval(NumeratorDenominator(a));
	return r[1];
});
addRule($$`Denominator(a_Integer)`, $$`1`);
addRule($$`Denominator(a_Rational)`, ({ a }) => Integer(a[2][1]));
addRule($$`Denominator(a_)`, ({ a }) => {
	const r = Eval(NumeratorDenominator(a));
	return r[2];
});
...
addRule($$`Expand(List(a__))`, $$`Map(Expand,List(a))`);
