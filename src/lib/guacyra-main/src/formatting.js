...
const { isNumeric, NumeratorDenominator } = Algebra;
...
const formatEchelonSteps = (A, options = {}) => {
	let { method = reducedRowEchelonSteps, format = (m) => formatMatrix(m) } =
		options;
	let last;
	...
	for (let s of method(A)) {
		...
		if (s.op === "init") {
			...
			const l = latex($`AAAAA+${s.k}*BBBBB`)
				.replace("AAAAA", `L_{${s.i}}`)
				.replace("BBBBB", `L_{${s.ip}}`);
			tt = `L_{${s.i}} \\rightarrow ${l}`;
		}
		const ta = format(s.A);
